...
sealed record Configuration(string Game, float Gain, bool AdaptiveTriggers)
{
    public static Configuration Read()
    {
        ...
        return new(config["game"]?.GetValue<string>() ?? "", gain, adaptiveTriggers);
    }
    public void Save() => Files.Save(Files.At("config.json"), new { game = Game, gain = Gain, adaptive_triggers = AdaptiveTriggers });
}
